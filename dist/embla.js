...
(() => {
  ...
  function EventStore() {
    let listeners = [];
    function add(node, type, handler, options2 = {
      passive: true
    }) {
      let removeListener;
      if ("addEventListener" in node) {
        node.addEventListener(type, handler, options2);
        removeListener = () => node.removeEventListener(type, handler, options2);
      } else {
  ...
  function Engine(root, container, slides, ownerDocument, ownerWindow, options2, eventHandler) {
    const {
      align,
      axis: scrollAxis,
      direction,
  ...
  function EmblaCarousel(root, userOptions, userPlugins) {
    ...
    function createEngine(options3) {
      const engine2 = Engine(root, container, slides, ownerDocument, ownerWindow, options3, eventHandler);
  ...
  // node_modules/.pnpm/embla-carousel-autoplay@8.5.1_embla-carousel@8.5.1/node_modules/embla-carousel-autoplay/esm/embla-carousel-autoplay.esm.js
  var defaultOptions2 = {
    active: true,
    breakpoints: {},
    delay: 4e3,
  ...
  function Autoplay(userOptions = {}) {
    ...
    function init(emblaApiInstance, optionsHandler) {
      ...
      options2 = optionsAtMedia(allOptions);
      if (emblaApi2.scrollSnapList().length <= 1) return;
      jump = options2.jump;
      destroyed = false;
      delay = normalizeDelay(emblaApi2, options2.delay);
      const {
        eventStore,
        ownerDocument
      } = emblaApi2.internalEngine();
      const isDraggable = !!emblaApi2.internalEngine().options.watchDrag;
      const root = getAutoplayRootNode(emblaApi2, options2.rootNode);
      eventStore.add(ownerDocument, "visibilitychange", visibilityChange);
      if (isDraggable) {
        emblaApi2.on("pointerDown", pointerDown);
      }
      if (isDraggable && !options2.stopOnInteraction) {
        emblaApi2.on("pointerUp", pointerUp);
      }
      if (options2.stopOnMouseEnter) {
        eventStore.add(root, "mouseenter", mouseEnter);
      }
      if (options2.stopOnMouseEnter && !options2.stopOnInteraction) {
        eventStore.add(root, "mouseleave", mouseLeave);
      }
      if (options2.stopOnFocusIn) {
        emblaApi2.on("slideFocusStart", stopAutoplay);
      }
      if (options2.stopOnFocusIn && !options2.stopOnInteraction) {
        eventStore.add(emblaApi2.containerNode(), "focusout", startAutoplay);
      }
      if (options2.playOnInit && !documentIsHidden()) startAutoplay();
    }
    function destroy() {
    ...
    function next() {
      ...
      const kill = options2.stopOnLastSnap && nextIndex === lastIndex;
  ...
  // src/embla.ts
  var emblaNode = getAssertedHtmlElement(".embla");
  var options = { loop: false, align: "center", skipSnaps: true };
  var plugins = [Autoplay()];
  var emblaApi = EmblaCarousel(emblaNode, options, plugins);
  var slideNodes = emblaApi.slideNodes();
  var activeSlideElement = void 0;
  for (let i = 0; i < slideNodes.length; i++) {
    const slideEl = slideNodes.at(i);
    const slideElVideo = slideEl.querySelector("video");
    ...
    slideEl.addEventListener("click", () => {
      ...
      activeSlideElement?.querySelector("video")?.pause();
